...
#+OPTIONS:  skip:nil d:(HIDE) tags:not-in-toc author:nil date:nil creator:nil
#+LANGUAGE: en
...
- 0xVV is a random unsigned 8-bit number, chosen by the host, used as
  ...
  value flipped (see [[id:92c2e9d2-bf6c-4a8e-b732-0eed8cba2406][Key Usage]] section).
...
:PROPERTIES:
:ID:       92c2e9d2-bf6c-4a8e-b732-0eed8cba2406
:END:
...
The value sent as the host key in the key exchange will need to have
the nibbles flipped when this final key is calculated. For instance,
if the value sent to the ET312 during exchange is 0x12, when
calculating the key, the value used should be 0x21.
...
| [[id:90e753ea-ceff-4b85-a7c2-d1bad201809d][$40a2]]         | *Mode Switch Ramp Action at Max* (0xfc)                     |
...
The output for each channel can be gated (turned on and off continously) with a
variable speed determined by the value of the Gate Select.
...
As an example, The "Mode" waves has a single module for channel A (#11) and a
single module for channel B (#12) which uses the defaults as
above, but modifies them as follows.  Module 11:

- sets the MA knob range from ($4086) 0x01 - ($4087) 0x40, 
- sets the A Width step ($40bb) to 0x02
- sets the A Width select ($40be) to 0x41 (244Hz timer, rate is from MA knob).  So the A Width will go from 0x32 to 0xc8 in 0x02 steps at 244Hz, then back down, repeating.  Note though that the minimum width is actually 0x46, so the width value will "stick" at 0 for a little while before coming back up.
- sets the A Frequency max ($40b0) to 0x80
- sets the A Frequency select ($40b5) to 0x41 (244Hz timer, rate is from MA knob).  So the A Frequency will go from 0x09 to 0x80 in 0x01 steps at 244Hz, then back down, repeating.
...
- sets the B Width step ($41bb) to 0x03.  So the B Width will go from 0x32 to 0xc8 in 0x03 steps at the advanced parameter default rate, then back down, repeating.
- sets the B Frequency max ($41b0) to 0x40.  So the B Frequency will go from 0x09 to 0x40 in 0x01 steps, then back down, repeating.
- doesn't change the B Intensity variable (so will be static at 0xff)
