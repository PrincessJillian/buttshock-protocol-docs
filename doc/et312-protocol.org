...
As an example, The "Mode" waves has a single module for channel A (#12) and a
single module for channel B (#13) which uses the defaults as
above, but modifies them as follows.  Module 12:
